...
import { getReportData, getCategoryTrends, getCashflowTrends, getInvestmentAnalytics, clearAllInvestments, deleteInvestment, getOwnerBalances, getCreditCardSpending, updateMasterInvestment } from '../services/transactionService';
...
<TouchableOpacity key={cur} style={[styles.curBtn, currency === cur && { backgroundColor: cur === 'AED' ? colors.primary : colors.accentTeal }]} onPress={() => setCurrency(cur)}>
  ...
  const renderOwnerBalanceCard = () => {
    const items = [
      { label: 'Prathista', balance: ownerBalances.SELF || 0 },
      { label: 'Praveen', balance: ownerBalances.SPOUSE || 0 },
      { label: 'Other', balance: ownerBalances.OTHER || 0 }
    ];

    return (
      <View style={{ marginTop: 12, marginBottom: 12 }}>
        <Text style={[typography.sectionLabel, { marginBottom: 8 }]}>OWNER BALANCE</Text>
        <GlassCard style={{ borderRadius: 16 }} contentStyle={{ padding: 12 }}>
          {/* Table Header */}
          <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: colors.border, paddingBottom: 6, marginBottom: 6 }}>
            <Text style={{ flex: 1, fontFamily: 'Inter_700Bold', fontSize: 12, color: colors.textSecondary }}>OWNER</Text>
            <Text style={{ flex: 1.5, fontFamily: 'Inter_700Bold', fontSize: 12, color: colors.textSecondary, textAlign: 'right' }}>BALANCE</Text>
          </View>
          {/* Table Rows */}
          {items.map((item, idx) => (
            <View key={item.label} style={{ flexDirection: 'row', paddingVertical: 8, borderBottomWidth: idx < 2 ? 1 : 0, borderBottomColor: colors.border + '15', alignItems: 'center' }}>
              <Text style={{ flex: 1, fontFamily: 'Inter_700Bold', fontSize: 13, color: colors.text }}>{item.label}</Text>
              <Text style={{ flex: 1.5, fontFamily: 'Inter_600SemiBold', fontSize: 13, color: item.balance >= 0 ? colors.text : colors.danger, textAlign: 'right' }}>
                {item.balance < 0 ? '-' : ''}{currency} {fmt(Math.abs(item.balance))}
              </Text>
            </View>
          ))}
        </GlassCard>
