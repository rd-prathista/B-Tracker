...
export const addInvestment = (data) => {
  ...
  db.runSync(
    `INSERT INTO investment_contributions (investment_id, amount, currency, contribution_date, notes, payment_source, credit_card_id) VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [masterId, parseFloat(recurring_amount), currency, start_date, 'Initial Contribution', payment_source || 'Debit Card', credit_card_id || null]
  );

  return masterId;
};

export const addContribution = (investmentId, amount, date, notes = '', attachmentUri = null, paymentSource = 'Debit Card', creditCardId = null, funded_by = 'OTHER') => {
  const db = getDb();
  ...
  db.runSync(
    `INSERT INTO investment_contributions (investment_id, amount, currency, contribution_date, notes, attachment_uri, payment_source, credit_card_id, funded_by, is_archived) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0)`,
    [investmentId, parseFloat(amount), master.currency, date, notes, attachmentUri, paymentSource, creditCardId, funded_by]
  );
...
export const updateContribution = (id, { amount, date, notes, attachmentUri, paymentSource, creditCardId, owner, masterUpdates }) => {
  const db = getDb();
  ...
  if (owner !== undefined) { sets.push('funded_by = ?'); params.push(owner); }
...
// OB_DISABLED: The two exports below are used exclusively by the Opening Balance Wizard.
// They are preserved here for future redesign. Uncomment to re-enable.
/*
